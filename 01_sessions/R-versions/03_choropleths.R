...
# Working directory
here::i_am("01_sessions/R-versions/03_choropleths.R")
...
# Compute avoided_conversion in R with NA handling
df_merged <- df_merged %>%
  mutate(
    avoided_conversion = case_when(
      is.na(zonal_mean_2032) | is.na(zonal_mean_2022) ~ NA,  # Assign 0 if either value is NA
      (zonal_mean_2032 - zonal_mean_2022) >= 0 ~ 0,         # Assign 0 if difference is non-negative
      max_diff > min_diff ~ (abs(zonal_mean_2032 - zonal_mean_2022) - min_diff) / (max_diff - min_diff),  # Scaled value
      TRUE ~ 0                                              # Default to 0
    )
  )
...
df_merged <- df_merged %>%
  mutate(
    restoration = case_when(
      is.na(zonal_mean_2022) | is.na(zonal_mean_2017) ~ NA,  # Assign 0 if either value is NA
      (zonal_mean_2022 - zonal_mean_2017) >= 0 ~ 0,         # Assign 0 if difference is non-negative
      max_diff > min_diff ~ (abs(zonal_mean_2022 - zonal_mean_2017) - min_diff) / (max_diff - min_diff),  # Scaled value
      TRUE ~ 0                                              # Default to 0
    )
  )
...
gdf_final <- st_as_sf(gdf_final) # Convert back to an sf object if needed


# Check for invalid geometries
st_is_valid(gdf_final)

# Find problematic geometries
invalid_geometries <- gdf_final[!st_is_valid(gdf_final), ]
...
# If needed, set the CRS
st_crs(gdf_final)
st_crs(gdf_final) <- 4326 # Replace 4326 with the appropriate EPSG code
st_crs(gdf_final) <- st_crs(borders)
...
# Save the plot
# ggsave("../04_results/land_use_change_2022_2032.png", dpi = 300, width = 10, height = 8)

#TODO: cantons overlay smth wrong with the layer 


# if (is.na(st_crs(cantons))) {
#   cantons <- st_set_crs(cantons, 4326)
# }
# cantons <- st_transform(cantons, raster_crs)
# cantons <- st_make_valid(cantons)
# cantons <- st_union(cantons)
# cantons <- st_cast(cantons, "MULTIPOLYGON")
# cantons <- st_simplify(cantons, dTolerance = 0.001)
